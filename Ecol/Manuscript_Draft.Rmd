...
where *y* is year, *r* is region, *s* is grid, *n* is total number of grids in  the region, and $\hat{MCP}$ is the MCP of each species. In the case of the GoMa, region was defined as the $0.5^{\circ}$ x $0.5^{\circ}$ grid-cell within the specific NAFO divisional zones. For the IPHC analysis, region was defined as the Commission's regulatory areas (Figure \@ref(fig:ocFigure1)). I then averaged the values in three time periods (*t*) to reduce temporal model sensitivity. Thus, I computed the regional percentage change in MCP ($\Delta MCP$) as follows:
...
```{r ocFigure2, eval = T, echo = F, fig.cap= "Percentage change of MCP for species managed by the IPHC for mid 21st century (2041-2060) relative to present 2005-2014 under a A) high emission scenario and B) low emission scenario. Labels marked with * represent regions where models do not agree in direction of change."}
...
```{r FigureS11, eval = F, echo = F, fig.cap = "Percentage change of MCP for species managed by the IPHC for end-century (2081–2100) relative to 2005–2014 under a A) high emission scenario and B) low emission scenario. Labels marked with * represent regions where models do not agree in direction of change."}
